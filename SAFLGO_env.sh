...
if [ -e "$TMP_DIR/distance.cfg.txt" -a -e "$TMP_DIR/BBtargets.txt" ]
then
  distance_size=$(wc -c <"$TMP_DIR/distance.cfg.txt")
  ...
  then
    ...
    if [ $distance_size -lt 5 ]
    then
      echo -e "${YELLOW}$0: ${OFF}${RED}Distance file is empty. Please try again!${OFF}"
      unset CC CXX
      return 0
    else
  ...
  if [[ ! -e "$TMP_DIR/distance.cfg.txt" ]]
  then
    echo -e "${YELLOW}$0: ${OFF}${RED}Can't find${OFF}${YELLOW} distance.cfg.txt${OFF}${RED} in the${OFF}${YELLOW} $TMP_DIR${OFF}${RED} folder"
    echo -e "${YELLOW}$0: ${OFF}${RED}Please try again!${OFF}"
    return 0
  else
