...
checkers=(
 ...
 #"alpha.core.CallAndMessageUnInitRefArg"  #Check for logical errors for function calls and Objective-C message expressions (e.g., uninitia                                            lized arguments, null function pointers, and pointer to undefined variables)
 ...
 #"alpha.core.TestAfterDivZero"            #Check for division by variable that is later compared against 0. Either the comparison is usele                                            ss or there is division by zero.
 ...
 "alpha.security.taint.TaintPropagation"   #Generate taint information used by other checkers
 #"alpha.unix.BlockInCriticalSection"      #Check for calls to blocking functions inside a critical section
 ...
 "alpha.unix.cstring.BufferOverlap"        #Checks for overlap in two buffer arguments
 #"alpha.unix.cstring.NotNullTerminated"   #Check for arguments which are not null-terminating strings
